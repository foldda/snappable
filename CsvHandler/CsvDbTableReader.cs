...
namespace Foldda.DataAutomation.CsvHandler
{
    ...
    public class CsvDbTableReader : BaseCsvDbTableHandler
    {
        ...
        protected override void ProcessTabularRecord(TabularRecord triggerRecord, Rda processingContext, DataContainer outputContainer, CancellationToken cancellationToken)
        {
            try
            {
                Log($"Database read triggered by {triggerRecord}");

                DbTableConnectionConfig config;
                //testing if the trigger contains 'download instructions' in its context,
                //if not, use the handler's local settings
                try
                {
                    HandlerEvent trigger = new HandlerEvent(triggerRecord.ToRda());
                    config = new DbTableConnectionConfig(trigger.EventContextRda);
                }
                catch
                {
                    Log($"Trigger event has no target-table-reading instrcution in its context, local (DB) config settings are used.");
                    config = LocalConfig;
                }

                TabularRecord.MetaData containerLabel = new TabularRecord.MetaData() { SourceId = config.DbTableName };
                if(_columnDefinitions.Count > 0)
                {
                    //if query specified columns
                    containerLabel.ColumnNames = _columnDefinitions.Keys.ToArray();
                }
                else
                {
                    //if query "*"
                    containerLabel.ColumnNames = _targetTableSchema.ToArray();
                }
                outputContainer.MetaData = containerLabel;
                    ...
                    using (OleDbDataReader reader = selectCommand.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            ...
                            for (int colIndex = 0; colIndex < reader.FieldCount; colIndex++)
                            {
                                ...
                                {
                                    //can also apply formatting based on "reader.GetFieldType(col)"
                            ...
                            Log($"Got row - '{string.Join(",", columnValues)}'");

                            outputContainer.Add(new TabularRecord(columnValues).ToRda());
                        }
                    }
