...
namespace Foldda.Automation.HandlerDevKit
{
    ...
    /// 
    /// </summary>
    public class HandlerController : IHandlerManager
    {
        ...
        public ILoggingProvider HandlerLogger => HandlerModel;
        ...
        public List<IHandlerEventListener> HandlerEventListeners { get; }
        ...
        {
            if (!evnt.EventSourceId.Equals(Handler.UID))
            {
                Handler.ProcessInboundMessage(evnt, cancellationToken);
            }
        }
        ...
        public BasicDataHandler CreateHandlerInstance(string fullTypeName, string handlerAssemblyName)
        {
            ...
            try
            {
                ...
                //#endif
                result = Activator.CreateInstance(type, [this]) as BasicDataHandler;
                if (result == null) 
                { 
        ...
        public IDataHandler Handler { get; private set; }  //the handler it manages
                    ...
                    do
                    {
                        ...
                        foreach (var item in handlerInput)
                        {
                            ...
                            if (item is RecordContainer inputContainer)
                            {
                                ...
                                await Handler.ProcessPipelineRecordContainer(inputContainer, cancellationToken);
                            }
                ...
                try
                {
                    do
                    {
                        ...
                        foreach (var item in inboundMessages)
                        {
                            ...
                            {
                                await Handler.ProcessInboundMessage(handlerMessage, cancellationToken);
                            }
                            else
                            {
        ...
        public virtual Task InitializeHandler(CancellationToken cancellationToken)
        {
            return Task.Run(async () =>
            {
                try
                {
                    ...
                    await Handler.Init(cancellationToken);
                }
                catch (Exception e)
                {
        ...
        private Task StartHandler(CancellationToken cancellationToken)
        {
            var t = Task.Run(() =>
            {
                try
                {
                    ...
                    Handler.Setup(HandlerModel.HandlerConfig);
            ...
            return Task.Run(async () =>
            {
                try
                {
                    do
                    {
                        ...
                        Handler?.ProcessInboundMessage(new MessageRda.HandlerEvent(this.Id, DateTime.Now, Rda.NULL), cancellationToken);
