...
Cross-company software components aren't interchangeable because, to effectively collaborate, they need to establish and maintain a "compatible" data model during their data exchange. However, due to each company's software development having its own evolving business requirements, data exchange between independently-built software is normally through dedicated custom interfaces, which are expensive to build and to manage.[^1]
...
Lego inspires us that a robust and flexible modular architecture shall have a standardized, capable, yet simple way of connecting the modules. Snappable is a library that provides generic component interfacing - for connecting component modules in a modular software architecture. Just like Lego joints enable building Lego models using various Lego blocks, software components can always be "connected" by Snappable and exchange data, regardless of the differences in their data models. Such a feature is the key to making "Lego-like" generic software components because it allows components to evolve on their data model without breaking their data-exchanging interface to the other components.

Another analogy for describing Snappable is the breadboard, which is used for building prototype electronic circuits by connecting electronic components and ICs via conductive metal wires and pins. Components in a breadboard are interchangeable as long as they have pins that are compatible with standard-sized plugs (holes) on the breadboard, and the breadboard's function is simply connecting the components and transmitting electrical signals between them. In an analogy comparing Snappable compatible software components to the electrical components on a breadboard, the Snappable-provided interface is like the pin holes of the breadboard, and the data between the software components is like the electrical signals of a breadboard circuit.
