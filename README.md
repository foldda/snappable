...
Cross-company software components aren't interchangeable because each company has its own evolving business requirements in its software development, and it's difficult to establish and maintain "compatible" data models with the other companies, which is required for cross-company software to exchange data and collaborate. Data exchange between independently-developped software normally require dedicated custom-built interfaces, but it is inpractical between software components.[^1]
...
To have generic and interchangeable software components, we'll need a way of low-cost, easy-to-maintain component-level interfacing and data exchanging that is resistant or immute to individual components' business requirements and internal data model changes.
...
Lego inspires us that a robust and flexible modular architecture shall have a simple and generic, yet capable way of connecting the modules. For a modular software architecture, this means generic component interfacing - that is, in the software world, a standardized system and method for software component modules to exchange data, 

Snappable is a library for generic component interfacing. Just like the generic Lego joints that enable joining and Lego blocks when building Lego models, the generic interface provided by Snappable software allows any compatible components to connect and exchange data between each other, without restricting the components from independently changing their data models. Through the "static", maintainance-free interface provided from Snappable, compatible software components become "Lego-like" - meaning they can be independently developed, and freely evolve, while always being able to exchanging data with the other Snappable-compatible components.
