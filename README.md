...
For cross-company software components to be interchangeable, it means these components from different sources need to interact and exchange data inside an application, which implies using an agreed data model for the data exchange. This is difficult if components are developed independently by different companies, because each company would have its own business requirements that determine the data models to be used, which are often evolving. If two independently developed applications must collaborate and exchange data, it's usually implemented as an afterthought and typically requires building and maintaining dedicated middleware to bridge incompatible data models, and such an approach is impractical at the software components' granularity.

So, for building interchangeable software components, we need a simple and effective interface for independently developed components to exchange data without incurring the high cost of bridging incompatible data models.
