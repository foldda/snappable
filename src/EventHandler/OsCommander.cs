...
namespace Foldda.Automation.EventHandler
{
    ...
    public class OsCommander : BasicSnappable
    {
        ...
        public OsCommander(ISnappableManager manager) : base(manager)
        {
        }
                        ...
                        using (AutoResetEvent errorWaitHandle = new AutoResetEvent(false))
                        {
                                ...
                                errorWaitHandle.WaitOne(timeOutMiliSec))
                            {
                                ...
                                if (process.ExitCode == 0)
                                {
                                    ...
                                    Manager.PostHandlerOutboundMessage(new CommandOutputRecord(this.GetType().Name, DateTime.Now, new Rda() { ScalarValue = output.ToString() }));
                                }
                                else
                                {
