...
namespace Foldda.Automation.EventHandler
{
     ...
     * 
     */
    public class WebhookConnector : BasicSnappable
    {
        ...
        public WebhookConnector(ISnappableManager manager) : base(manager)
        {
        }
        ...
        public override Task<int> ProcessPipelineRecordContainer(RecordContainer inputContainer, CancellationToken cancellationToken)
        {
            if (inputContainer.Records.Count > 0)
            {
                ...
                try
                {
                    ...
                    Manager.PipelineOutputDataStorage.Receive(new MessageRda.HandlerEvent(UID, DateTime.Now, Rda.NULL));  //create a dummy event 
                }
                catch (Exception e)
                {
