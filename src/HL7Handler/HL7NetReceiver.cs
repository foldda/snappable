...
namespace Foldda.Automation.HL7Handler
{
    public class HL7NetReceiver : BaseHL7Handler
    {
        ...
        public HL7NetReceiver(ISnappableManager manager) : base(manager) { }
                    ...
                    Task dataCollectorTask = Task.Run(async () =>
                    {
                        try
                        {
                            do
                            {
                                ...
                                ackProducer.CollectedBufferredContainers(Manager.PipelineOutputDataStorage, MINIMAL_CONTAINER_INACTIVE_AGE_SEC);
